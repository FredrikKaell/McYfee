...
def activate_monitors():
    """
    Function for ACTIVATING monitors
    """
    print("Showing activated scrapers")
    allowed = []
    ...
    while True:
        ...
        try:
            if selection == "q":
                return
            if int(selection) in allowed:
                set_monitor_status(int(selection), 0)
                return
            else:
...
def deactivate_monitors():
    """
    Function for DEACTIVATING monitors
    """
    print("Showing deactivated scrapers")
    allowed = []
    monitors = fetch_monitors(False)

    if not monitors:
        print("No deactivated monitors")
        return

    for post in enumerate(monitors):
        allowed.append(post['id'])
        print(f"id : {post['id']} - {post['name']}")
    print("Print 'q' to quit current selection")
    while True:
        selection = input("Select id to activate: ")
        try:
            if selection == "q":
                return
            if int(selection) in allowed:
                set_monitor_status(int(selection), 1)
                return
            else:
                print("Unknown selection.")
        except:
